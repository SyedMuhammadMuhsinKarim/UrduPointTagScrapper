...
class Get_Elements:
    ...
    def get_headlines(self): # O(1)
        headlines_tags = Get_Content(self.content_posts, 'h1', 'class', 'fs24 lh48 urdu ar rtl').content()
        headlines = [headlines_tags[i].text if headlines_tags[i] != None else np.nan for i in range(len(headlines_tags))]
        return headlines

    def get_category(self): # O(n^2)
        category_tag = Get_Content(self.content_posts, 'div', 'class', 'tagcloud rtl ar urdu').content()
        category = [category_tag[i].find_all('a') if category_tag[i] != None else [] for i in range(len(category_tag))] # O(n)
        categories = []
        categories_links = []
